...
def lottery_status():
    with db_lock:
        count = cursor.execute("SELECT COUNT(*) FROM lottery").fetchone()[0]
        return count > 0
...
def handle_text(msg):
    ...
    if text=="🎥 Видео көру":
        if not check_subscription(user_id):
            ...
            return
        with db_lock:
            balance, progress = cursor.execute("SELECT balance, progress_video FROM users WHERE user_id=?",
                                              (user_id,)).fetchone()
    ...
    # Лотереяға қатысу
        ...
        with db_lock:
            ...
            if exists:
                ...
                return
            cursor.execute("INSERT INTO lottery (user_id, joined_at) VALUES (?,?)",
    ...
    if text=="💰 Бонус беру":
        ...
        for pid,uploader,ctype,file_id,file_path in pendings:
            ...
            if ctype=="video":
                if file_path and os.path.exists(file_path):
                    with open(file_path,"rb") as f:
                        bot.send_video(user_id,f,caption=f"#{pid} {ctype} жіберген: {uploader}", reply_markup=kb)
                else:
                    bot.send_video(user_id,file_id,caption=f"#{pid} {ctype} жіберген: {uploader}", reply_markup=kb)
            else:
                if file_path and os.path.exists(file_path):
                    with open(file_path,"rb") as f:
                        bot.send_photo(user_id,f,caption=f"#{pid} {ctype} жіберген: {uploader}", reply_markup=kb)
                else:
                    bot.send_photo(user_id,file_id,caption=f"#{pid} {ctype} жіберген: {uploader}", reply_markup=kb)
        return
