...
CHANNEL_CHECK = "@kazakcombots"  # канал тексеру
...
def handle_text(message):
    ...
    if text == "🎥 Видео":
        with db_lock:
            ...
            rows = cursor.execute("SELECT id, file_id FROM videos ORDER BY id ASC").fetchall()
            if not rows:
    ...
    if text == "➕ Видео/Фото қосу":
        bot.send_message(user_id, "Файлты осы чатқа жіберіңіз. (Админға модерацияға кетеді)")
        return
...
def handle_media(message):
    ...
    kb_text = f"Пайдаланушы {user_id} жіберді: {ctype}\n✅ Растау / ❌ Тастау"
    bot.send_message(ADMIN_ID, kb_text + f"\napprove_{pid} / reject_{pid}")

    bot.send_message(user_id, f"✅ Файл модерацияға жіберілді. Админ мақұлдағаннан кейін хабарланады.")
